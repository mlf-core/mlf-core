...
class VersionBumper:
    ...
    def bump_template_version(self, new_version: str, project_dir: Path) -> None:
        ...
        ct_cfg_path = f'{str(project_dir)}/mlf_core.cfg' if str(project_dir).startswith(str(Path.cwd())) else \
            f'{str(Path.cwd())}/{project_dir}/mlf_core.cfg'
                ...
                for line in old_file:
                    ...
                    if ('<<MLF-CORE_NO_BUMP>>' not in line and not section == 'bumpversion_files_blacklisted') or '<<MLF-CORE_FORCE_BUMP>>' in line:
                        ...
                        if tmp != line:
                            ...
                            print(f'[bold red]- {line.strip().replace("<!-- <<MLF-CORE_FORCE_BUMP>> -->", "")}\n'
