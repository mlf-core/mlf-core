...
from mlf_core.common.version import (load_mlf_core_template_version,
                                     load_project_template_version_and_handle)
from mlf_core.config.config import ConfigCommand
from mlf_core.create.create import choose_domain
from mlf_core.create.github_support import (create_sync_secret, decrypt_pat,
...
class TemplateSync:
    ...
    def commit_template_changes(self):
        ...
        try:
            ...
            if files_to_commit:
                ...
                Popen(
                    ["git", "commit", "-m", "mlf-core sync", *files_to_commit],
                    stdout=PIPE,
                    stderr=PIPE,
    ...
    def push_template_branch(self):
        ...
        try:
            ...
            self.repo.remotes.origin.push(
                refspec=f"mlf_core_sync_v{self.new_template_version}:mlf_core_sync_v{self.new_template_version}"
            )
        except git.exc.GitCommandError as e:
    ...
    def check_pull_request_exists(self) -> bool:
        ...
        for pull_request in repo.get_pulls(state="open"):
            ...
            if "Important mlf-core template update" in pull_request.title:
                print(
                    "[bold blue]Detected open, but outdated mlf-core sync PR. Closing it in favor of the newest sync PR!"
                )
    ...
    def should_run_sync(self) -> bool:
        ...
        1.) First check, whether the user disabled sync in the mlf_core.cfg file with sync_enabled = False in the [sync] section
        ...
        try:
            ...
            if (
                len(level_item) != 1
                or "sync_level" not in level_item[0][0]
                or not any(level_item[0][1] == valid_lvl for valid_lvl in ["major", "minor", "patch"])
            ):
                print(
                    "[bold red]Your mlf_core_sync_level section is missconfigured. Make sure that it only contains one item named mlf_core_sync_level "
        ...
        # mlf_core.cfg file was not found or has no section sync_level
            ...
            print(
                "[bold red]Could not read from mlf_core.cfg file. "
                "Make sure your specified path contains a mlf_core.cfg file and has a sync_level and sync section!"
            )
            sys.exit(1)
    ...
    def get_blacklisted_sync_globs(self) -> list:
        ...
        # mlf_core.cfg file was not found or has no section called sync_files_blacklisted
            ...
            print(
                "[bold red]Could not read from mlf_core.cfg file. Make sure your specified path contains a mlf_core.cfg file and has a "
    ...
    def update_sync_token(project_name: str, gh_username: str = "") -> None:
        ...
        updated_sync_token = mlf_core_questionary_or_dot_mlf_core(
    ...
    def has_template_version_changed(project_dir: Path) -> Tuple[bool, bool, bool, str, str]:
        ...
        log.debug(
            f"Projects template version is {template_version_last_sync} and mlf-core template version is {current_mlf_core_template_version}"
        )
        ...
        return (
            ...
            str(current_mlf_core_template_version),
        )
    ...
    def sync_load_template_version(handle: str) -> str:
        ...
        log.debug(
            f"Using available templates file from {available_templates_path} to load current mlf-core template version."
        )
